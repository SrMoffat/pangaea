...
import { CartContext, CurrencyContext } from "../../state";
import { productImageStyles, productNameStyles, addCartButtonStyles } from "./productStyles";

const Product = (props) => {
    const { dispatch } = useContext(CartContext);
    const { state: { current } } = useContext(CurrencyContext);
    ...
    return (
        <div>
          ...
          <div style={{ marginBottom: 15 }} className="product-price">{`From: ${current} ${price}`}</div>
